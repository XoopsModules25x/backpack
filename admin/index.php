...
switch ($mode) {
	case POST_SELECT_MODULE_FORM: {
		...
		$mod_selections = make_module_selection($select_dirname);
	...
	case POST_DB_SELECT_FORM: {
		$select_dirname = isset($_GET['dirname']) ? $_GET['dirname'] : 0;
		$mod_selections = make_module_selection($select_dirname,1);
	...
	case POST_SELECT_TABLES_FORM: {
		...
		backup_data($tablename_array, $backup_structure, $backup_data, $filename, $cfgZipType);
		if ( $download_count == 1 ){
